...
<div class="col col-sm-3 col-xs-12">
                            <h6 class="title">Add Thermo Forming Matchines <span></span></h6>

                        </div> 
                        </div>
  <form action="{{ route('thermo_mc_add') }}" method="POST" class="modern-form" id="add_admin_form">
  @csrf
...
@endif
    <div class="row">
      ...
      <div class="col-md-6">
        ...
        <div class="form-group">
          <label for="main_matirial">Main Product Material Type</label>
        ...
        <div class="form-group">
          <label for="comp">Secondary Product Material Type</label>
                ...
                <table class="table" id="myTable">
                        ...
                        @foreach ($admin as $admin)
                            ...
                            <td><i class="fa fa-circle {{$admin['mc_status']==0?'text-danger':'text-success'}}"></i> </td>
                                <td>{{$admin['thermo_mc_no']}}</td>
                                <td>{{$admin['thermo_mc_name']}}</td>
                                <td>{{$admin['thermo_main_matirial']}}</td>
                                <td>{{$admin['thermo_second_matirial']}}</td>
                                <td>
                               <form action="{{ url('start-stop-thermo-machine/'.$admin['thermo_mc_id']) }}" method="post">
                      @csrf
                      <button class="btn btn-{{$admin['mc_status']==0?'success':'warning'}} btn-sm" type="submit"><i class="fa {{$admin['mc_status']==0?'fa-play':'fa-stop'}}"></i> {{$admin['mc_status']==0?'Start':'Stop'}}</button>
                                ...
                                <td>
                              <a href="{{ 'delete_thermo_mc/'.$admin['thermo_mc_id'] }}" class="btn btn-danger btn-sm" onclick="return styledConfirm('Are you sure you want to delete this user?')">Delete</a>
                               </td>
                            </tr>
                            @endforeach
                        </tbody>
